...
fdone <- list.files(file.path(carob_path, "data", "compiled"), pattern="latest_metadata.csv$", full=TRUE)
...
github <- paste0("https://github.com/carob-data/carob/blob/master/scripts/", x$carob_group, "/", suri, ".R")
script <- paste0('<a href="', github, '">R script</a>')

rep <- paste0("http://carob-data.org/reports/", x$carob_group, "/", x$dataset_id, ".html")
frep <- basename(list.files(file.path(carob_path, "data", "clean"), pattern="html$", recursive = TRUE))
i <- x$dataset_id %in% gsub(".html", "", frep)
#x$links <- paste0(x$carob_group, 
...
#x$dataset <- paste0(x$citation, " [", x$carob_group, "] ", script)
x$dataset <- paste0(x$citation, " [", x$carob_group, "] ")
...
z <- x[, c("data_organization", "dataset", "crops", "countries", "project")]
z$crops[is.na(z$crops)] <- ""
z$countries[is.na(z$countries)] <- ""
z$project[is.na(z$project)] <- ""
js <- jsonlite::toJSON(list(data=z))
writeLines(js, "../_static/providers.json")

vocal::set_vocabulary("github:controvoc/terminag")
