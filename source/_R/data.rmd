...
*Carob* aggregates data sets into groups. Below is a table with the current groups and the number of datasets and records in each group. We also show these numbers for the datasets that have a Creative Commons (CC) [license](licenses.html). We have processed `r sum(x$Datasets)` data sets containing a total of `r sum(x$Records)` records. You can also browse the [datasets](done.html) that we processed.
