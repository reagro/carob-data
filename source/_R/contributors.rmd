...
get_authors <- function(d) {
	...
	p$p <- gsub("ATEKU", "Ateku", p$p)
	p$p <- gsub("BAUDRON|BAUDRO", "Baudron", p$p)
	p$p <- gsub("CHAMBERLIN", "Chamberlin", p$p)
	p$p <- gsub("GAMEDA", "Gameda", p$p)
	p$p <- gsub("GEBREKIDAN", "Gebrekidan", p$p)
	p$p <- gsub("WEULLOW", "Weullow", p$p)
	p$p <- gsub("SILVA", "Silva", p$p)
	p$p <- gsub("SIDA", "Sida", p$p)
	p$p <- gsub("DESALEGN", "Desalegn", p$p)
	...
	p <- p[!grepl("ABC|International Center|Alliance of Bioversity|CIAT|Institut|ILRI|IFPRI|IRRI|IWMI|CIMMYT|ICRISAT|IITA|ICRAF|ISRIC|LUANAR|TARI|MSU|Ministry|SARI|University|ZARI|CRS|TLC", p$p), ]
	p$p <- gsub("  ", " ", p$p)
	p$p <- trimws(p$p)
	p <- p[p$p !="", ]
	...
	p$p <- gsub("Dorado Betancourt, Hugo A.|Dorado, Hugo Andres", "Dorado Betancourt, Hugo A.", p$p) 
	p$p <- gsub("Shapiro, C. A.|Shapiro, Charles", "Shapiro, Charles A.", p$p)
	p$p <- gsub("Kaizzi, Kayuki$", "Kaizzi, Kayuki C.", p$p)
	p$p <- gsub("Valentine KARARI", "Karari, Valentine", p$p)
	p$p <- gsub("Vagen, Tor-Gunnar", "Vågen, Tor-Gunnar", p$p)
	p$p <- gsub("Joao Vasco", "João Vasco", p$p)
	p$p <- gsub("Jo˜ao Vasco", "João Vasco", p$p) # odd case but needed
	p$p <- gsub("Kamara, Alpha Y.", "Kamara, Alpha", p$p)
	p$p <- gsub("KUBOJA, Nicholaus", "Kuboja, Nicholaus", p$p)
	p$p <- gsub("PITTAKI, Zampela", "Pittaki, Zampela", p$p)

	tab <- as.data.frame(table(p$p))
	colnames(tab) <- c("name", "datasets")
	tab$alpha <- toupper(substr(tab$name, 1, 1))
	tab$alpha[tab$alpha=="Ö"] <- "O"
	tab
}
...
vocal::set_vocabulary("github:controvoc/terminag", quiet=TRUE)
...
The *Carob* scripts were written by the following `r nrow(cartab)` persons:
...
`r nrow(intab)` organizations contributed to the *Carob* data:
...
`r nrow(autab)` persons contributed to the *Carob* data:
