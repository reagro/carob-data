...
get_authors <- function(d) {
	...
	i <- grepl(",|ICRAF|ABC|ICRISAT|SARI|IITA|IRRI|ISRIC|CIMMYT|TLC|University|IWIN|ARI|Program|ILRI|Davis", p$p)
	...
	p <- p[!grepl("ABC|ILRI|CIMMYT|ICRISAT|IITA|ICRAF|TARI|University of California", p$p), ]
	p$p <- gsub("  ", " ", p$p)
	p$p <- trimws(p$p)
	p <- p[p$p !="", ]
